...
function generateId() {
  return `medi-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
}

async function importProducts() {
  const csvPath = '/Users/mac/Downloads/Medicontur_Products_Catalog.csv';
  const csvContent = fs.readFileSync(csvPath, 'utf-8');
  const records = parseCSV(csvContent);

  console.log(`Found ${records.length} Medicontur products to import\n`);

  const partnerId = 'e8940dd7-2393-43ec-b647-a31ba633fb3f'; // MEDICONTUR
  const categoryId = 'ophthalmology'; // Ophthalmology - IOLs are ophthalmic products
  ...
  for (const record of records) {
    try {
      ...
      console.error(`Error importing ${record['reference']}:`, error.message);
      errors++;
    }
  }
  ...
  console.log(`\nTotal Medicontur products in database: ${count}`);
