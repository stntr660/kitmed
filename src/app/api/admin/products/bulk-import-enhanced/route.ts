...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (fileDownloads.length > 0) {
      console.log(`Starting file downloads for ${fileDownloads.length} products`);
      ...
      for (const productDownload of fileDownloads) {
        ...
        for (const urlInfo of productDownload.urls) {
          try {
            ...
            console.log(`Downloaded ${urlInfo.type} for ${productDownload.productRef}: ${downloadResult.url}`);
